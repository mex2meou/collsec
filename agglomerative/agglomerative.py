...
for i in range(0, num_tests):
    ...
    # choose the sample file for a quick test
    # window_logs = pd.read_pickle(data_dir + "sample.pkl")
    
    # otherwise, load the data files
    ...
    l_blacklists = dict(); l_whitelists = dict()
    ...
    for n_clusters in clusters_values:
        ...
        connectivity = kneighbors_graph(X_train, n_neighbors=n_clusters+4, include_self=False)
        estimator = AgglomerativeClustering(n_clusters=n_clusters, connectivity=connectivity, linkage='average', affinity='cosine')
        ...
        for subset in clusters:
            ...
            # compute the gub blacklist/whitelist for the cluster
            gub_bl = set()            
            gub_wl = set()
...
df_stats.to_pickle("../results/agglomerative_stats.pkl")