...
def plot_tpr(method, stats):
    ...
    plt.xlabel('Percentage of Pairs', fontsize=14)
...
def plot_fn_increase(method, stats):
    ...
    for k in np.unique(stats["k"]):
        if k!= 0:
            k_values.append(k)
            k_stats = stats[stats.k == k]
            int_fn_impr.append(k_stats['fn_int_incr'].mean())
...
def plot_f1_measure(method, stats):
    ...
    legend = plt.legend(numpoints=1, loc='lower right', fontsize='small') 
...
if __name__ == '__main__':
    ...
    df['fn_gub_incr'] = df.apply(compute_fn_gub_impr, axis=1)
    df['fn_int_incr'] = df.apply(compute_fn_int_impr, axis=1)
